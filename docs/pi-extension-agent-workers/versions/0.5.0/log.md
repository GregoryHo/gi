...
- Completed M1. Added bounded complete child results with private log fallback, actual child system-prompt and exact model/thinking pass-through, a default/per-request turn cap with distinct `turn_limit`, tested minimal resource boundaries, and setup/prompt/missing-final observability coverage. Verification passed: 134 package tests, package typecheck, and `git diff --check`.
- Started M2 Versioned worker runtime protocol. The implementation must use the same service instance as commands/tools/widgets and remain limited to in-process `pi.events` capability/start/status/wait/cancel/profile operations.
- Completed M2. Added protocol v1 server/client contracts, correlated capability/start/status/wait/cancel/profile operations, explicit confirmation gating, read-only authority narrowing, complete bounded result copies, readiness/disposal, and deterministic runtime-unavailable/response-timeout errors. Verification passed: 143 tests, typecheck, pack dry-run, and `git diff --check`.
